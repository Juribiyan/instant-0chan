  ...
  deleteItems: function(items) {
    items.forEach(item => {
      if (item.action == 'delete_file') {
        $(`figure[data-fileid=${item.id}]`)
        .addClass('deleted').css({'opacity': '0.5'})
      }
      if (item.action == 'delete_post') {
        let $thread = $(`#delform [id^=thread${item.id}]`)
        , $reply = $(`#delform [id^=reply${item.id}]`)
        if ($thread.length) {
          $thread.addClass('deleted').css({'opacity': '0.5'})
        }
        else if ($reply.length) {
          $reply.addClass('deleted').css({'opacity': '0.5'})
        }
      }
    })
  }
