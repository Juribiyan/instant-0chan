...
var updater = {
  ...
  showNewThreads: function() {
    ...
    this.newThreads.forEach(thr => {
      HTMLoader.getThread(this_board_dir, thr, null, (err, posts) => {
        ...
        document.querySelector('#delform').insertAdjacentHTML('afterBegin', posts+'<br clear="left" /><hr />');
      })
    })
    this.newThreads = [];
  },
  ...
  bpageNotify: function(data) {
    ...
    if(data.room == 'newthreads') {
      updater.newThreads.push(data.post_id);
      ...
      if(!$target.find('.fresh-replies').text(_l.newReplies+': '+(Number(($target.find('.fresh-replies').text().split(':')[1]))+1)).length)
        ...
        .find('.fresh-replies').click(function(e) {
          ...
          newposts.get({
            ...
            onError: () => document.location.href = `${ku_boardspath}/${res.board}/res/${data.room}.html#${data.post_id}`,
            silent: true,
            timestamp: data.timestamp
          })
...
function add_mob_menu() {
  ...
  cloud20.getBoards().then(b20 => {
    ...
    htm += `<a href="#" class="mm-item" onclick="
