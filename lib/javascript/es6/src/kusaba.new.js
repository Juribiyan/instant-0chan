  ...
  addFile: function(file, extra, form, fromClipboard=false) {
    ...
    form.querySelector('.fe-sort-wrapper').insertAdjacentHTML('beforeEnd', `
      <div class="file-entry${fromClipboard ? ' fe-name-hidden' : ''}" data-index="${index}" title="${file.name}">
        ...
        <button class="clear-filename fe-pop-button icon-wraping-button" title="Clear name"><svg class="icon b-icon">
          <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#i-hide-name"></use>
        </svg></button>
        <button class="remove-file fe-pop-button icon-wraping-button" title="Remove file"><svg class="icon b-icon">
          <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#i-x"></use>
        </svg></button>
        <button class="spoiler-file fe-pop-button icon-wraping-button" title="Spoiler on/off"><svg class="icon b-icon">
    ...
    if (count) {
      if (! $counter.length) {
        ...
        if (! document.hasFocus()) {
          ...
          let $freshReplies = $target.append('<a href="/'+ this_board_dir +'/res/'+thread+'.html" class="salient fresh-replies">'+_l.newReplies+': <span class="fresh-replies-number"></span></a>')
          .find('.fresh-replies')
