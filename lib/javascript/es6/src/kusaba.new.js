...
function updateActiveForm() {
  ...
  let closest = findClosest('.qrf-floating.collapsed, .postform textarea')
  if (closest) {
    lastActiveForm = closest
    return
  }
}
...
function clonePostForm(anchorID, preview) {
  ...
  /*if (1) {*/ //TODO: add simplified form setting? 
...
var Captcha = {
  ...
  initForm: function(form) {
    ...
    form.querySelector('input[name=captcha]').onclick = this.onFieldClick.bind(this)
  },
  _state: 'init',
  get state() {
    return this._state
...
var PostPreviews = {
  ...
  _mouseout: function(nodelay=false) {
    var preview = $(this);
    if ($(this).is('a')) {
      if (this.predelay) {
...
function readyset() {
  ...
  if(!isTouch) {
    ...
    offClick.push(ev => {
      menu_show('_off_');
      document.querySelectorAll('[id^=preview]').forEach(pre => PostPreviews._mouseout.bind(pre)(true))
    })
  ...
  .on('click', '.reflink a', function(ev) {
    ...
    , postBody = ref._findParent('.postnode').querySelector('.postbody')
  ...
  let postformArea = document.querySelector('#postform textarea')
  if (postformArea)
    lastActiveForm = postformArea
...
function unwrapEmbed($fig) {
  ...
  if ($iw.data('site') == "Soundcloud") {
    let start = $iw.data('start')
    $iw.append(`<iframe src="https://w.soundcloud.com/player/?url=https%3A%2F%2Fapi.soundcloud.com%2Ftracks%2F${encodeURI(code)}" ${iframeOptions}></iframe>`)
  }
