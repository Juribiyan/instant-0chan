...
const updater = {
  ...
  refreshNewRepliesCount(thread, timestamp=null) {
    ...
    if (count) {
      if (! $counter.length) {
        $target.append('<a href="/'+ this_board_dir +'/res/'+thread+'.html" class="salient fresh-replies">'+_l.newReplies+': <span class="fresh-replies-number"></span></a>')
        .find('.fresh-replies').click(function(e) {
          e.preventDefault()
          if (this.__alreadyLoading)
            return;
          this.__alreadyLoading = true
          this.insertAdjacentHTML('afterBegin', '<div class="spinner"></div>')
          newposts.get({
            threadid: thread,
            onSuccess: () => {
              clearCounter()
