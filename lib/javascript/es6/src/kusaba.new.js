...
function getnewposts(threadid) {
  ...
  let $neposts_get = $('#newposts_get')
  $neposts_get.find('a').hide()
  let $msg = $neposts_get.find('span')
  if (!$msg.length) {
    $msg = $(`<span></span>`).prependTo($neposts_get)
  }
  $msg.text(`${_l.loading}...`).show()
  HTMLoader.getThread(this_board_dir, +threadid, [+lastpost,Infinity], (err, posts) => {
    $msg.hide()
    $neposts_get.find('a').show()
    if (posts) {
      $('.replies')[0].innerHTML += posts
    }
    else {
