...
var _messages = {
  en: {
    ...
    deleteItems: "Delete selected posts and files",
  ...
  ru: {
    ...
    deleteItems: "Удалить выбранные файлы и посты",
...
const Ajax = {
  ...
  deleteItems: function(fd) {
    ...
    if (!confirm(_l[`delete${kind}`]+'?')) return;
...
function readyset() {
  ...
  // ID highlighting
  $('body').on('click', '.hashpic', function() {
    $('.highlight').removeClass('highlight')
  ...
  $('body').on('click', 'button.delete-file', function(e) {
    ...
    let $this = $(this)
    if (e.ctrlKey) {
      $('body').addClass('delete-multiple')
      $this.parent('figcaption').find('input.delete-file').prop('checked', true).trigger('change')
      return
    }
    let fd = new FormData()
    fd.append('delete-file[]', $this.parents('figure')[0].dataset['fileid'])
