...
class MediaViewer {
  ...
  collapse() {
    ...
    if (document.body.contains(this.currentThumb)) {
      ...
      , transX = (thumbBCR.x + thumbBCR.width/2) - this.viewer.clientWidth/2
  ...
  createMediaElement(underImageSrc, imgw, imgh, imgurl, trans='', isVideo=false) {
    return `<div class="media-item ${isVideo ? 'loaded' : ''}" ${trans}>
      ...
      <${isVideo ? 'video controls autoplay loop' : 'img'} src="${imgurl}" class="mv-over" ${isVideo 
        ? `></video><img class="video-event-overlay" src="data:image/svg+xml;charset=utf-8,%3Csvg xmlns%3D'http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg' width='${imgw}' height='${imgh-50}'%2F%3E">`
