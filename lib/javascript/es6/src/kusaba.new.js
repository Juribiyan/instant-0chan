...
var updater = {
  ...
  init: function () {
    ...
    if(ispage) {
      ...
      .emit('subscribe', liveupd_sitename+$('input[name=board]').val()+':'+$('input[name=replythread]').val())
    }
  },
  ...
  bpageNotify: function(data) {
    // Do nothing if it is my post
    if (data.token && Ajax.postToken && data.token==Ajax.postToken) return;
    if(data.room == 'threads') {
      updater.newThreads.push(data.new_thread_id);
