...
var HiddenItems = {
  ...
  hideThread: function(id, inCatalog=false) {
    ...
    $('#thread' + id + this_board_dir).slideUp()
  },
...
const richFileInput = {
  ...
  recalcForm: function($form) {
    let count = $form.find('.file-entry').length
    $form.toggleClass('fda-non-empty', count > 0)
    .toggleClass('fda-form-full', count >= maxfiles)
  },
  ...
  init: function() {
    ...
    if (this.enabled) {
      ...
      .on('click', '.remove-file', function(ev) {
        ...
        let $form = $entry.parents('form')
        $entry.remove()
        _this.recalcForm($form)
      })
      ...
      .on('click', '.add-files', function(ev) {
        ev.preventDefault()
        $(this).parents('form').find('input[type=file]')[0].click()
      })
      ...
      $('.drop-area').prev().each(function() { $(this).removeClass('noscript') })
      $('.drop-area').remove()
      $('body')
      .on('click', '.remove-file', function(ev) {
        ev.preventDefault()
...
function readyset() {
  ...
  if(!isTouch) {
    $('#postclone').drags()
    .find('input, textarea, select, label, .fe-sort-wrapper').mousedown(function(e) {
      e.stopPropagation()
    })
...
const soundSprite = {
  ...
  // delay: 0.0025, //only presents in mp3 file, off must be edited to have the same delay
  current: null,
  el: null,
  init: function() {
    document.body.insertAdjacentHTML('beforeend', `<audio id="sound-sprite" style="display:none">
      <source src="${ku_boardspath}/images/sprite.mp3">
      <source src="${ku_boardspath}/images/sprite.ogg">
    </audio>`)
    this.el = document.querySelector('#sound-sprite')
    let _this = this
    this.el.addEventListener('timeupdate', function() {
      // console.log(this.currentTime)
      if (this.currentTime >= _this.current[1]) {
        this.pause()
      }
    }, false)
    $('.sound-test').click(function(ev) {
      ev.preventDefault()
      _this.play($(this).parent().find('select[name=sound]').val())
    })
  },
  play: function(id=null) {
    if (!id) return;
    let sprite = id=='random' ? _.sample(this.data) : this.data[id]
    if (sprite && sprite.length) {
      this.current = sprite
      this.el.currentTime = sprite[0]
      this.el.play().catch(e => _.noop)
    }
  }
}
...
function resetForm(form) {
  ...
  form.querySelectorAll('.file-entry .remove-file').forEach(rf => rf.click())
}
