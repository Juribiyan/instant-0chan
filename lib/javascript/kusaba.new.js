...
var _ = _messages.hasOwnProperty(locale) ? _messages[locale] : _messages.ru;
...
function captcha_show() { 
    if (captcha_shown == 0 || captcha_rotten == 1) { 
        $(".captchaimage").attr("src", ku_boardspath+'/captcha.php?'+Math.random()).css({'opacity': '1'}); 
...
function readyset() {
    ...
    $('body').on('click', '.uib-tx', function() {
        ...
        head.js('http://latex.codecogs.com/editor3.js', function() {
            OpenLatexEditor(target,'phpBB','', false, '','full');
        });
        return false;
    });
    //new quick reply
    ...
    if($('#de-panel').length) {
        //first, make settings panel invisible so user won't notice
        injectCSS('#de-content-cfg {opacity: 0!important}', 7);
        //if anything changed, must reload; But keep the preferences for the board page
        var dcxt_changed = 0;
        //open a panel
    ...
    if(!ispage && react_ena) {
        head.load(react_api + '/socket.io/socket.io.js', function() {
            if(typeof io !== 'undefined') {
                console.log('Successfully connected to reactive API server '+react_api);
                var ajax_posting = false;
                if($('#de-panel').length) {
                    ...
                    if(dcxt_changed) location.reload();
                }
                ...
                socket.on('update', function(data) {
                    ...
                    if(typeof data.token !== undefined) {
                        if($('input[name=token][value='+ data.token +']').length) {
                            ...
                            if(ajax_posting) doUpdate = false;
                        }
                    }
                    if(doUpdate) getnewposts();
                ...
                if(!ajax_posting) $('#postform, #postclone').on('submit', function() {xsend($(this).attr('id')); return false;});
            }
            else console.log('Error connecting to reactive API server '+react_api);
        });
    }

    if($('#de-panel').length) {
        //close the dcxt settings window
        $('#de-btn-settings')[0].click();
        //Make panel visible again
        setTimeout(function() {removeCSS(7)}, 1000);
    }
    ...
    $(window).scroll(function() {
        if($(window).scrollTop() + $(window).height() > $(document).height() - 100 && $('body').height() / $('#postform').height() > 5) {
           $('#delform').after($('.postarea'));
        }
        else if ($(window).scrollTop() < 100) {
            $('#delform').before($('.postarea'));
        }
    });
...
function removeCSS(order) {
    iter(document.styleSheets, function(sheet) {
        if(sheet.href && sheet.href.split('/css/')[1].split('?')[0] === injectDestination) {
