...
function readyset() {
    ...
    if(interfaceType) {
        ...
        $('#boardlist_header').after('<div id="interface-notifier" class="content-background reflinkpreview"><div class="ifcn-text">'+_.impliedTouch+'</div><div class="incn-buttons">'+
            '<button class="ifcnbtn" onclick="javascript: localStorage.setItem(\'interfaceType\', \'touch\'); $(\'#interface-notifier\').fadeOut(\'fast\'); return false">'+_.okay+'</button>&nbsp;'+
            '<button class="ifcnbtn" onclick="javascript: localStorage.setItem(\'interfaceType\', \'desktop\'); location.reload(); return false">'+_.forceDesktop+'</button></div></div>');
    }
    else {
...
//    $('#postform').clone().attr({'id': 'postclone', 'onsubmit': "return checkcaptcha('postclone');"}).appendTo('body').hide().addClass('reflinkpreview content-background qreplyform').find('.blotter').remove();
    ...
    if(react_ena && typeof io !== 'undefined') {
        ...
        $('#postform input[type="submit"]').after('<img class="sending-loading" src="/images/loading.gif">')
...
var embedLinks = {
    sites: [
        ...
        {id: 'vimeo',   rx: /[\w\W]*vimeo\.com\/([\w\W]*)[\w\W]*/ },
