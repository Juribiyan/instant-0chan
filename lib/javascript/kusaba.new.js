...
function delandbanlinks($scope, force) {
  ...
  $scope.find('span[id^=dnb]').each(function(index,element) {
    ...
    newhtml += 'id=' + dnbinfo[2] + '" title="'+_l.delete+'" onclick="return confirm(\''+_l.deletePost+'?\');"><img src="'+ku_boardspath+'/css/icons/blank.gif" class="delete spritebtn sb-l"><\/a>';
    ...
    newhtml +='id=' + dnbinfo[2] + '&postid=' + dnbinfo[2] + '" title="'+_l.delandban+'" onclick="return confirm(\''+_l.deletePost+'?\');"><img src="'+ku_boardspath+'/css/icons/blank.gif" class="dandb spritebtn sb-c"><\/a>';
    newhtml +='<a href="' + ku_cgipath + '/manage_page.php?action=bans&banboard=' + dnbinfo[1] + '&banpost=' + dnbinfo[2] + '" title="'+_l.ban+'"><img src="'+ku_boardspath+'/css/icons/blank.gif" class="ban spritebtn sb-r"><\/a></span>&nbsp;';

    if (dnbinfo[3] == 'y') {
      newhtml += '<span class="btngroup"><a href="' + ku_cgipath + '/manage_page.php?action=stickypost&board=' + dnbinfo[1] + '&postid=' + dnbinfo[2] + '" title="'+_l.stickthread+'" ><img src="'+ku_boardspath+'/css/icons/blank.gif" class="stick spritebtn sb-l"><\/a><a href="' + ku_cgipath + '/manage_page.php?action=unstickypost&board=' + dnbinfo[1] + '&postid=' + dnbinfo[2] + '" title="'+_l.unstickthread+'" ><img src="'+ku_boardspath+'/css/icons/blank.gif" class="unstick spritebtn sb-r"><\/a></span>&nbsp;';
      newhtml += '<span class="btngroup"><a href="' + ku_cgipath + '/manage_page.php?action=lockpost&board=' + dnbinfo[1] + '&postid=' + dnbinfo[2] + '" title="'+_l.lockthread+'" ><img src="'+ku_boardspath+'/css/icons/blank.gif" class="lock spritebtn sb-l"><\/a><a href="' + ku_cgipath + '/manage_page.php?action=unlockpost&board=' + dnbinfo[1] + '&postid=' + dnbinfo[2] + '" title="'+_l.unlockthread+'" ><img src="'+ku_boardspath+'/css/icons/blank.gif" class="unlock spritebtn sb-r"><\/a></span>';
    }
...
function readyset() {
  ...
  if(interfaceType) {
    ...
    $('#boardlist_header').after('<div id="interface-notifier" class="content-background reflinkpreview"><div class="ifcn-text">'+_l.impliedTouch+'</div><div class="incn-buttons">'+
      '<button class="ifcnbtn" onclick="javascript: localStorage.setItem(\'interfaceType\', \'touch\'); $(\'#interface-notifier\').fadeOut(\'fast\');">'+_l.okay+'</button>&nbsp;'+
      '<button class="ifcnbtn" onclick="javascript: localStorage.setItem(\'interfaceType\', \'desktop\'); location.reload();">'+_l.forceDesktop+'</button></div></div>');
  }
  else {
    $('#js_settings').prepend('<a title="'+_l.impliedDesktop+'" href="javascript: localStorage[\'interfaceType\']=\'touch\'; location.reload();">'+_l.returnTouch+'</a><br>');
  }
  ...
  $('<div id="viewlink"></div>').addClass('content-background reflinkpreview qreplyform').html(
    ...
    '<a href="#" onclick="javascript:$(\'#viewlink\').hide();return false;" title="'+_l.Close+'"><img style="vertical-align: top;" src="'+ku_boardspath+'/css/icons/blank.gif" border="0" class="closebox spritebtn"></a>'
  ).hide().appendTo('body');
...
function showreplies(root) {
  ...
  $(root).find('.postnode').each(function(index, element) {
    ...
    if (replies.length > 0) { 
      repliesStrings[postid] = '<div class="replieslist"><br />'+_l.replies+': ';
...
var catalog = {
  ...
  build: function(data) {
    ...
    _.each(data, function(entry, i) {
      _.each(['id', 'reply_count', 'bumped', 'replied', 'reply_count', 'timestamp', 'page', 'locked', 'stickied'], function(prop) {
    ...
    _.each(this.model, function(thread) {
      // if(thread.hasOwnProperty('hidden')) console.log(thread.id, thread.hidden)
    ...
    if(!thread.processed) {
      ...
      if(!thread.file || thread.file === 'removed') {
        ...
        if(_.includes(this.fileTypes.image, thread.file_type)) {
          ...
          +'</a>';
          /*if(thread.file_type === 'webm') 
            thread.bigThumb = '<video controls src="src/'+thread.file+'.webm"></video>';
          else */thread.bigThumb = 
          '<img src="thumb/'+thread.file+'s.'+(thread.file_type === 'webm' ? 'jpg' : thread.file_type)+'">' /*+
          (thread.file_type === 'webm' ? '<svg class="icon cat-play-big" data-src="src/'+thread.file+'.'+thread.file_type+'"><use xlink:href="#i-play"></use></svg>' : '')*/;
        }
      ...
      if(thread.file && thread.file !== 'removed')
        thread.bigThumb = '<a target="_blank" href="'+thread.url+'" class="bigThumb">'+thread.bigThumb/*+expanderBtn*/+'</a>';
      // thread.textModeBigThumb = '<div class="bigThumb">'+thread.bigThumb+'</div>';
      // thread.imgModeBigThumb = '<a href="'+thread.url+'" class="bigThumb">'+thread.bigThumb+'</a>';
  ...
  buildIndicators: function(thread) {
    ...
    thread.indicatorsCombined = '<div class="indicators">'+burger+'<span class="i-layer-1">'+pin+lock+hide+'</span><span class="i-layer-2">'+del+and+ban+'</span></div>';
    return thread;
  },
  layouts: {
