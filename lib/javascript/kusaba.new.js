...
function captcha_show() { 
    if (captcha_shown == 0 || captcha_rotten == 1) { 
        ...
        $('.captchawrap').css({'opacity': '1'}).animate({'width': 0}, captchaTimeout).css('overflow', 'visible');
...
function refreshCaptcha() {
    ...
    $('.captchaimage').attr('src', ku_boardspath+'/captcha.php?'+Math.random());
    $('.captchawrap').stop().css({'opacity': '1', 'width': 150}).animate({'width': 0}, captchaTimeout).css('overflow', 'visible');; 
    $(".captcha_status").hide();
